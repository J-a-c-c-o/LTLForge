...
impl Philosopher {
    ...
    fn pick_up_left(&mut self, forks: &mut [bool]) -> bool {
        ...
        if self.configuration.allowed_right && matches!(self.stage, Stage::InbetweenRight) && !forks[self.fork_left] {
            forks[self.fork_left] = true;
    ...
    fn pick_up_right(&mut self, forks: &mut [bool]) -> bool {
        if self.configuration.allowed_left && matches!(self.stage, Stage::InbetweenLeft) && !forks[self.fork_right] {
...
pub fn compute_reachable_states_and_deadlocks(n: usize, configurations: Vec<PhilosopherConfiguration>) -> (HashSet<(Vec<Philosopher>, Vec<bool>)>, HashSet<(Vec<Philosopher>, Vec<bool>)>) {
    let philosophers: Vec<Philosopher> = (0..n).map(|i| Philosopher::new(i, n, configurations[i].clone())).collect();
...
pub fn build_dining_philosophers(n: usize, configurations: Vec<PhilosopherConfiguration>) -> PetriNet {
    ...
    for i in 0..n {
        ...
        builder = builder.add_arc(format!("l{}", i), format!("inbetweenL{}", i));
        builder = builder.add_arc(format!("inbetweenL{}", i), format!("r{}", i));
